...
export interface ShopifyAnalysis {
  ...
  top_product_types: { type: string; count: number }[];
...
function detectTheme(html: string): string {
  const $ = cheerio.load(html);
  // Common theme meta or comments
  const themeComment = html.match(/Shopify Theme: ([^\n]+)/i);
  if (themeComment) return themeComment[1].trim();
  ...
  // Common known themes
  if (html.includes("Dawn")) return "Dawn";
  if (html.includes("Debut")) return "Debut";
  if (html.includes("Brooklyn")) return "Brooklyn";
  if (html.includes("Minimal")) return "Minimal";
  if (html.includes("Narrative")) return "Narrative";
