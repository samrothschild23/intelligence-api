...
router.get("/maps/search", async (req: Request, res: Response) => {
  ...
  if (!google_key || typeof google_key !== "string") {
    res.status(400).json({
      ...
      detail:
        "Provide your Google Places API key. Get one at https://console.cloud.google.com",
    });
    return;
  }

  const maxNum = max ? parseInt(max as string, 10) : 20;
...
router.get("/maps/leads", async (req: Request, res: Response) => {
  ...
  if (!industry || typeof industry !== "string") {
    res.status(400).json({
      ...
      example:
        "/maps/leads?industry=plumbers&location=Denver+CO&min_score=60&google_key=YOUR_KEY",
    });
    return;
  }
  ...
  const minScoreNum = min_score ? parseInt(min_score as string, 10) : 60;
